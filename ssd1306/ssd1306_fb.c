...
static int ssd1306_init_display(struct ssd1306_par *par)
{
        ...
        write_reg(par, 0x00); //--set low column address
        write_reg(par, 0x10); //--set high column address
        
        write_reg(par, 0x40); //--set start line address
        
        write_reg(par, 0xb0); //--set page address
        
        write_reg(par, 0x81); //--set contrast control register
        write_reg(par, 0xff);
        
        write_reg(par, 0xa1); //--set segment re-map 127 to 0   a0:0 to seg127
        write_reg(par, 0xa6); //--set normal display
        
        write_reg(par, 0xc9); //--set com(N-1)to com0  c0:com0 to com(N-1)
        
        write_reg(par, 0xa8); //--set multiples ratio(1to64)
        write_reg(par, 0x3f); //--1/64 duty
        
        write_reg(par, 0xd3); //--set display offset
        write_reg(par, 0x00); //--not offset
        
        write_reg(par, 0xd5); //--set display clock divide ratio/oscillator frequency
        write_reg(par, 0x80); //--set divide ratio
        
        write_reg(par, 0xd9); //--set pre-charge period
        write_reg(par, 0xf1);
        
        write_reg(par, 0xda); //--set com pins hardware configuration
        write_reg(par, 0x12);
        
        write_reg(par, 0xdb); //--set vcomh
        write_reg(par, 0x40);
        
        write_reg(par, 0x8d); //--set chare pump enable/disable
        write_reg(par, 0x14); //--set(0x10) disable
        // write_reg(par, 0x10);        //--set(0x10) disable
        write_reg(par, 0xaf); //--turn on oled panel
...
static int ssd1306_request_one_gpio(struct ssd1306_par *par,
...
static int ssd1306_request_gpios(struct ssd1306_par *par)
{
        int rc;
        pr_debug("%s, configure from dt\n", __func__);
        
        rc = ssd1306_request_one_gpio(par, "reset-gpios", 0, &par->gpio.reset);
        if (rc)
                return rc;
        rc = ssd1306_request_one_gpio(par, "dc-gpios", 0, &par->gpio.dc);
        if (rc)
                return rc;
        rc = ssd1306_request_one_gpio(par, "blk-gpios", 0, &par->gpio.blk);
        if (rc)
                return rc;
        rc = ssd1306_request_one_gpio(par, "cs-gpios", 0, &par->gpio.cs);
        if (rc)
                return rc;
...
static int write_vmem(struct ssd1306_par *par, size_t offset, size_t len)
{
        u16 *vmem16;
